...
export const getFeedPosts = async (req, res) => {
	try {
		const posts = await Post.find({ author: { $in: [...req.user.connections, req.user._id] } })
			.populate("author", "name username profilePicture headline").sort({ createdAt: -1 });
...
//get user posts with user id 
export const userPosts = async (req,res) => {
	try {
		const { userId } = req.params;
		const posts = await Post.find({author:userId}).populate('author','name type profilePicture bio').sort({createdAt:-1});
		return res.status(200).json({status:"success",posts});
	} catch (error) {
		console.error('userPosts Error:', error);
		res.status(500).json({ status: "error", message: 'Failed to get posts' });
	}
}